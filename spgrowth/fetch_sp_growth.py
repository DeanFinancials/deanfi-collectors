#!/usr/bin/env python3
"""
S&P Growth Extractor - SEC EDGAR data extractor for annual & quarterly financials.

Unified collector that processes S&P 500 tickers and generates both:
- sp100growth.json (S&P 100 subset)
- sp500growth.json (full S&P 500)

This avoids duplicate API calls by processing all 500 tickers once and filtering
the output into the appropriate files.
...
This collector fetches fundamental growth metrics:
...
Usage:
    python fetch_sp_growth.py                    # Process S&P 500, output both files
    python fetch_sp_growth.py --output ./output  # Custom output directory
    python fetch_sp_growth.py --sp100-only       # Only output sp100growth.json
    python fetch_sp_growth.py --sp500-only       # Only output sp500growth.json
"""
...
# Import S&P universes from shared modules
...
def build_readme_section(universe: str, config: Config) -> dict:
    """Build the _README section for a growth JSON file."""
    return {
        "title": f"{universe} Growth Metrics",
        "description": f"Financial growth metrics extracted from SEC EDGAR filings for {universe} companies",
...
def save_growth_output(
    ...
    """
    Save growth data to JSON, optionally filtering to specific tickers.
    
    Args:
        results: List of all CompanyData results
        ticker_filter: If provided, only include tickers in this set
        output_path: Path to save the JSON file
        universe_name: Name for metadata (e.g., "S&P 100", "S&P 500")
    ...
    metadata = {
        ...
        "ticker_count": len(filtered_results),
        "successful_extractions": success_count,
        "universe": universe_name
    }
    
    combined = {
        "_README": build_readme_section(universe_name, config),
        "metadata": metadata,
        "companies": companies,
    }
    
    save_json(combined, output_path, config.indent)
    print(f"[ok] Wrote {universe_name}: {output_path} ({success_count}/{len(filtered_results)} tickers)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract annual & quarterly revenue/EPS from SEC EDGAR for S&P 500 companies. "
                    "Generates both sp100growth.json and sp500growth.json from a single run."
    )
    parser.add_argument("--config", "-c", default=None, help="Path to config.yml")
    parser.add_argument("--output", "-o", default=None, help="Output directory")
    parser.add_argument("--sp100-only", action="store_true", help="Only output sp100growth.json")
    parser.add_argument("--sp500-only", action="store_true", help="Only output sp500growth.json")
    ...
    output_sp100 = not args.sp500_only
    output_sp500 = not args.sp100_only
    ...
    if args.sp100_only:
        tickers = sorted(sp100_tickers)
        print(f"[info] Processing S&P 100 only ({len(tickers)} tickers)")
    else:
        # Process full S&P 500 - this covers all SP100 tickers too
        tickers = sp500_tickers
        print(f"[info] Processing S&P 500 ({len(tickers)} tickers, includes {len(sp100_tickers)} S&P 100)")
    
    if not tickers:
        raise SystemExit("No tickers to process - universe is empty")
    ...
    # Save SP100 output (filtered from full results)
    if output_sp100:
        sp100_path = config.output_dir / "sp100growth.json"
        save_growth_output(results, sp100_tickers, sp100_path, "S&P 100", config)
    
    # Save SP500 output (all results)
    if output_sp500:
        sp500_path = config.output_dir / "sp500growth.json"
        save_growth_output(results, None, sp500_path, "S&P 500", config)
    
    print(f"\n[ok] Successfully extracted data for {success_count}/{len(tickers)} total tickers")
